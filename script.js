...
function renderSkills() {
  ...
  const sections = [
    { title: "Toolkit", copy: "The tools I reach for to analyze, design, and ship work.", items: data.skills.toolkit },
    { title: "Capabilities", copy: "How I work through ambiguity, stakeholders, and process.", items: data.skills.capabilities }
  ];
...
function renderProjectCard(project) {
  ...
  const isCaseStudy = project.category === "cs";
  if (isCaseStudy) card.classList.add("project-card--case-study");
  ...
  body.innerHTML = `
    <div class="project-body-top">
      <p class="project-kicker">${isCaseStudy ? "Case Study" : project.category === "pm" ? "Management (Product/Project)" : "Developer"}</p>
      <h3>${project.title}</h3>
      ${project.description ? `<p class="section-copy">${project.description}</p>` : ""}
    </div>
  `;
  ...
  card.addEventListener("click", (event) => {
    ...
    window.location.href = project.caseStudyPage || `project.html?project=${encodeURIComponent(project.slug)}`;
  });
...
function renderContact() {
  ...
  $("#contactLocation").textContent = data.contact.location || "Remote";
}
